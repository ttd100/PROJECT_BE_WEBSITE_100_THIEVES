...
@Autowired
    ...
    public ResponseEntity<?> createProductStyle(@Valid @RequestBody ProductStyle productStyle){
        ...
        if (productStyleService.existsByName(productStyle.getNameProductStyle())){
    ...
    public ResponseEntity<?>searchNameProStyle(@PathVariable String name){
        ...
        return new ResponseEntity<>(productStyleService.findAllByNameContaining(name),HttpStatus.OK);
    }
}
